	...
	class Validation {
		...
		*/
		function __construct($aForm) {
			if ( empty($aForm['login'])
				|| !$this->checkLength($aForm['login'], 2, 50)
				|| !(preg_match("/[a-zA-Z\x20]/", $aForm['login'])
				|| preg_match("/[а-яА-ЯЁё\x20]/u", $aForm['login'])))  {
				$this->bErrorName = true;
			} else {
				$this->bErrorName = false;
				$this->aResult['login'] = $this->cleanString($aForm['login']);
			}
