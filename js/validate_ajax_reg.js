        ...
        $.ajax(
            {
                ...
                success: function (data) {
                    ...
                    //-----------------------------------------------------
                    else if (data[2].err_login_dubble !== '') {
                        $(".invalid-feedback").remove();
                        elem_l.after('<div class="invalid-feedback">' + data[2].err_login_dubble + '</div>');
                        $("#login").toggleClass('is-invalid', true);
...
//------------Снимаем блокировку если нет ошибок-------------------------------------------------------
    ...
    $('#button_reg').on('click', function (event) {
        ...
        if (isValid_login && isValid_email && isValid_password && isValid_password_confirm){
            $(".badge-warning").remove();
            $("#form_reg").off("submit");
        }else {
            event.preventDefault();
